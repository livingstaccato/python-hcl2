    ...
    def serialize(
        ...
        if options.preserve_heredocs:
            if context.inside_dollar_string:
                # An argument or operand is expression source, so the heredoc
                # goes back as written. It keeps the newline after its closing
                # marker: the token that follows has to start the next line,
                # and `upper(<<EOF\nfoo\nEOF)` does not parse. Quoting it
                # instead made a multi-line string, which OpenTofu rejects (#338).
                return heredoc
